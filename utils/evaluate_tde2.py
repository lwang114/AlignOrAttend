...
if __name__ == '__main__':
  ...
  parser.add_argument('CONFIG', type=str, default='../config/speechcoco.json')
  ...
  unfiltered_pred_box_file = '{}/val2014/mscoco_val_bboxes_rcnn.json'.format(data_dir)
  gold_box_file = '{}/val2014/mscoco_val_bboxes.txt'.format(data_dir)
  keep_id_file = '{}/val2014/mscoco_val_split.txt'.format(data_dir)

  if args.task == 0:
    alignment_files = []
    for fn in os.listdir(exp_dir):
      if 'alignment' in fn and 'json' in fn and not 'train' in fn:
        alignment_files.append('{}/{}'.format(exp_dir, fn))
        
    # if not os.path.isfile(pred_file):
    result_file = '{}/alignment_results.txt'.format(exp_dir)
    res_f = open(result_file, 'w')
    if not args.segment:
        caption_file = '{}/val2014/mscoco_val_text_captions.txt'.format(data_dir)
        pred_file = '{}/predicted_annotated_boxes.txt'.format(exp_dir)
        gold_file = '{}/gold_annotated_boxes.txt'.format(exp_dir)
    ...
    # if not os.path.isfile(gold_file):
        ...
        caption_file = '{}/val2014/mscoco_val_phone_captions_segmented.txt'.format(data_dir)
        segment_file = '{}/val2014/mscoco_val_word_phone_segments.txt'.format(data_dir)
        pred_file = '{}/predicted_segment_annotated_boxes.txt'.format(exp_dir)
        gold_file = '{}/gold_segment_annotated_boxes.txt'.format(exp_dir)
        for pred_alignment_file in alignment_files:
    ...
    result_file = '{}/token_results.txt'.format(exp_dir)
    res_f = open(result_file, 'w')
    caption_file = '{}/val2014/mscoco_val_text_captions.txt'.format(data_dir)
    pred_file = os.path.join(exp_dir, 'predicted_annotated_boxes.txt')
    gold_file = os.path.join(exp_dir, 'gold_annotated_boxes.txt')
    pred_alignment_file = os.path.join(exp_dir, 'alignment.json')
    _ = filter_boxes(caption_file, 
                     pred_alignment_file, 
                     unfiltered_pred_box_file,
                     gold_box_file,
    ...
    result_file = '{}/AP_results.txt'.format(exp_dir)
    res_f = open(result_file, 'w')
    caption_file = '{}/val2014/mscoco_val_text_captions.txt'.format(data_dir)
    pred_file = os.path.join(exp_dir, 'binary_alignment_scores.txt')
    gold_file = os.path.join(exp_dir, 'binary_alignment_labels.txt')
    pred_alignment_file = os.path.join(exp_dir, 'alignment.json')
    ...
    create_alignment_score_file(caption_file, 
                                ...
                                out_file=pred_file,
                                keep_id_file=keep_id_file)
    ap = alignment_average_precision(pred_file, gold_file, '{}/avg_precision.csv'.format(exp_dir))
    print('Average precision={}\n'.format(ap))
    res_f.write('Average precision={}\n'.format(ap))
  elif args.task == 3:
    result_file = '{}/retrieval_AP_results.txt'.format(exp_dir)
    res_f = open(result_file, 'w') 
    pred_file = os.path.join(exp_dir, 'similarity.npy')
    ap = retrieval_average_precision(pred_file, '{}/retrieval_avg_precision.csv'.format(exp_dir))
    print('Average precision={}\n'.format(ap))
